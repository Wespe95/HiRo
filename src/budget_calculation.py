...
def calculate_actions_in_budget(budget:float, measures_gdf:gpd.GeoDataFrame, data_path:str, actions:dict=None)->pd.DataFrame:
    
    bus_stops = measures_gdf.copy()
    ...
    bus_stops_after_actions["cost"] = bus_stops_after_actions["action"].apply(lambda x: actions[x]["costs"] if x is not None else None)
